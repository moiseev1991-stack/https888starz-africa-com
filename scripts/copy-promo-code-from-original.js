...
const promoCodeHtmlPath = path.join(PUBLIC, 'promo-code', 'index.html');
const ORIGINAL_URL = 'https://888starz-africa.com/promo-code/';
...
async function copyFullPageFromOriginal() {
    ...
    try {
        ...
        const promoCodeDir = path.dirname(promoCodeHtmlPath);
        fs.mkdirSync(promoCodeDir, { recursive: true });
        
        // Сохраняем полный HTML
        fs.writeFileSync(promoCodeHtmlPath, html, 'utf8');
        console.log(`✓ Полный HTML сохранен: ${promoCodeHtmlPath}`);
