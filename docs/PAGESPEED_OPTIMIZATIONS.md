...
**Проблема:** Третий сторонний ресурс с коротким кешем и блокировка рендера.

**Сделано:**
- К скриптам mc.yandex.ru и mc.yandex.com добавлен `async` — не блокируют отрисовку (PageSpeed).
- Кеш 1 час задаётся серверами Yandex, изменить нельзя.
