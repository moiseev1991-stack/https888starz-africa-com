...
class Creare_Twitter
{
	...
	private function getTimeAgo($time)
	{
  			...
  			/********************* Checking the times and returning correct value */
		   	if($theminutes < 60){
				if($theminutes < 1){
					$timemessage =  esc_html__("Less than 1 minute ago", 'zento');
				} else if($theminutes == 1) {
				 	$timemessage = sprintf( esc_html__("%s minute ago", 'zento'), $theminutes);
				} else {
				 	$timemessage = sprintf( esc_html__("%s minutes ago", 'zento'), $theminutes);
				}
			} else if($theminutes > 60 && $thedays < 1){
				 if($thehours == 1){
				 	$timemessage = sprintf( esc_html__("%s hour ago", 'zento'), $thehours);
				 } else {
				 	$timemessage = sprintf( esc_html__("%s hours ago", 'zento'), $thehours);
				 }
			} else {
				 if($thedays == 1){
				 	$timemessage = sprintf( esc_html__("%s day ago", 'zento'), $thedays);
				 } else {
				 	$timemessage = sprintf( esc_html__("%s days ago", 'zento'), $thedays);
				 }
			}
		return $timemessage;	
    }
